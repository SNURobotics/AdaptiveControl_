...
#define _SLICE_SIZE_		50
#define _STACK_SIZE_		50

static float _T[16];
SE3 T_localframe;
SE3 T_local2local_geom;
...
void srSimpleRenderer::_CompileLinkDLs( srgLink* psrgLink )
{
	...
	glNewList(psrgLink->m_iDL_RenderingMode, GL_COMPILE);
	{
		...
		{
		case srGeometryInfo::ELLIPSOID:
			srgDrawEllipsoid(SRG_FIGURE_FILL, rDim[0], rDim[1], rDim[2] , _SLICE_SIZE_, _STACK_SIZE_);
			break;
	...
	glNewList(psrgLink->m_iDL_DesignMode, GL_COMPILE);
	{
		...
		{
		/*case srGeometryInfo::ELLIPSOID:
			srgDrawEllipsoid(SRG_FIGURE_FEATURELINE, rDim[0], rDim[1], rDim[2], _SLICE_SIZE_, _STACK_SIZE_);
...
void srSimpleRenderer::_RenderLinks( void )
{
	...
	{
		T_localframe = _gLinks[i].m_pLink->GetFrame();
		T_local2local_geom = _gLinks[i].m_pLink->GetGeomInfo().GetLocalFrame();
		(T_localframe*T_local2local_geom).ToArray(_T);
