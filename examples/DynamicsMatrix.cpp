...
DynamicsMatrix::DynamicsMatrix(robot1* pRobot, AdaptiveControl* pAdaptiveControl) : mpRobot(pRobot), mpAdaptiveControl(pAdaptiveControl)
{
	...
	mVdot0 = MatrixXd::Constant(6 * mnJoint, 1, 0.0);
	...
	C = MatrixXd::Constant(mnJoint, 1, 0.0);
	...
	for (int i = 0; i < mnJoint; i++)
	{
		...
		se3 A_screw = Ad(mpRobot->m_link[i+1].m_Frame, Axis(JointAxis[0], JointAxis[1], JointAxis[2]));
		mvA[i] = A_screw;
		MatrixXd tmp(6, 1);
	...
	// Vdot_base
	Matrix4d BaseFrame;
	mpRobot->m_BaseLink->m_Frame.ToArray(BaseFrame.data());
	Vector4d Gravity = BaseFrame.inverse() * Vector4d(0,0,9.8,0); // opposite of gravity
	mVdot0.block<3, 1>(6 * (mnJoint - 1) + 3, 0) << Gravity.block<3, 1>(0, 0);
...
MatrixXd DynamicsMatrix::SmallAd(se3 V)
{
	...
	matSmallAd.block<3, 3>(0, 3) = CrossMatrix(v);
...
{
	mMatrixL = MatrixXd::Identity(6 * mnJoint, 6 * mnJoint);
	for (int i = 1; i < mnJoint - 1; i++)
	{
		mMatrixL.block<6, 6>(6 * i, 6 * (i - 1)) = BigAd(mvCurrentSE3[i + 1] % mvCurrentSE3[i]);
	}

	for (int i = 2; i < mnJoint - 1; i++)
	{
		for (int j = 0; j < i - 1; j++)
		{
			mMatrixL.block<6, 6>(6 * i, 6 * j) = mMatrixL.block<6, 6>(6 * (i - 1), 6 * (i - 2)) * mMatrixL.block<6, 6>(6 * (i - 2), 6 * (j - 1));
		}
	}
}
...
void DynamicsMatrix::Compute_adA()
{
	for (int i = 0; i < mnJoint; i++)
	{
		se3 A_i = mpRobot->m_joint[i].GetRevoluteJointState().m_rValue[1] * mvA[i];
...
{
	MatrixXd theta_dot(mnJoint, 1);
	for (int i = 0; i < mnJoint; i++)
	{
		theta_dot << mpRobot->m_joint[i].GetRevoluteJointState().m_rValue[1];
	}

	C = M = mMatrixA.transpose() * mMatrixL.transpose() * (mMatrixG*mMatrixL*mMatrix_adA*mMatrixGamma + mMatrixCbar) * mMatrixL * mMatrixA * theta_dot;
}

void DynamicsMatrix::GravitationalVector()
{
	N = M = mMatrixA.transpose() * mMatrixL.transpose() * mMatrixG * mMatrixL * mVdot0;
}
