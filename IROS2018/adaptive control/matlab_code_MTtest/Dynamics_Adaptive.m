function [dot_state_augmented] = Dynamics_Adaptive(t, state_augmented, robot, Stopper)
global state0_parameter AdaptEFonly NaturalAdaptation Euclidean_enhanced dt 
global TrajAmplitude Euclidean_projection hold_input_torque NoAdaptation
...
%%%%%%%%%%%%%%%%% 에러 처리%%%%%%%%%%%%%%%%%
if(max(max(isnan(MM))) == 1 || max(max(isinf(MM))) == 1)
    disp('MM is NaN or Inf')
    dot_state_augmented = nan;
    return;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(Stopper == 1)
    ...
    hold_input_torque = input_torque;
end
...
dot_state_joint(robot.nDOF+1:end,1) = pinv(MM)* ( -CC * state_joint_qdot  -gg  + hold_input_torque); % qddot

if(NoAdaptation)
    dot_state_parameter = zeros(robot.nDOF*10,1);
else % adaptative control
